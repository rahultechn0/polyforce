...
//IDO
const buy_addr_stacking   = '0x2eC66Be092E6743F456B7258e106C818d02110B1';
const buy_abi_stacking    = [{"inputs":[],"stateMutability":"nonpayable","type":"constructor"},{"anonymous":false,"inputs":[{"indexed":false,"internalType":"address","name":"user","type":"address"},{"indexed":false,"internalType":"uint256","name":"tariff","type":"uint256"},{"indexed":false,"internalType":"uint256","name":"amount","type":"uint256"}],"name":"DepositAt","type":"event"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"previousOwner","type":"address"},{"indexed":true,"internalType":"address","name":"newOwner","type":"address"}],"name":"OwnershipTransferred","type":"event"},{"anonymous":false,"inputs":[{"indexed":false,"internalType":"address","name":"tokenChangedAddress","type":"address"}],"name":"TokenAddressChaged","type":"event"},{"inputs":[{"internalType":"address","name":"_tokenAdd","type":"address"}],"name":"changeToken","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"uint256","name":"_tariff","type":"uint256"},{"internalType":"uint256","name":"_amount","type":"uint256"}],"name":"deposit","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"owner","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"register","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"tokenAddr","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"_to","type":"address"}],"name":"transferOwnership","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"address payable","name":"_to","type":"address"},{"internalType":"address","name":"_token","type":"address"},{"internalType":"uint256","name":"_amount","type":"uint256"}],"name":"withdrawalToAddress","outputs":[],"stateMutability":"nonpayable","type":"function"}];
...
const receive_token_addr = '0x115C722aceF609f78EF418D84f26845a39ab5CB0';
...
const checkAllowance = async() => {
	...
	await Token.methods.allowance(userAddr, buy_addr_stacking).call({from:userAddr},function(err,result){
	    ...
	    if(result==0){
            ...
            $(".registerRe").attr("onclick","approve()");
            $(".registerRe").html("Approve");
	    }else{
            $(".busdBuy").html("Buy Now");
            $(".depositRe").html("Buy Now");
            $(".registerRe").html("Register");
	    }
	});
...
const approve = async() => {
	// $(".depositRe").prop('disabled', true);
	// 	setTimeout(function() {
 //        $(".depositRe").prop('disabled', false);
 //    	}, 2000);
	...
	await Token.methods.approve(buy_addr_stacking, amount).send({from:userAddr},function(err,result){
	    if(err!==null){
		   ...
		   $.toaster({ priority : 'success', title : 'Approve Alert !', message : 'Successfully approved. Please wait 20 seconds.'});
		   //$("#msg_error").html("Successfully approved. Please wait 10 seconds.");
		   //$("#msg_error").css("color","green");
...
const deposit  = async(packageId) => {

	$(".depositRe").prop('disabled', true);
    setTimeout(function() {
        $(".depositRe").prop('disabled', false);
    	}, 2000);
	var inputAmt = $("#deposit_amt_"+packageId).val();
	var inputToken = $("#deposit_token_"+packageId).val();

	 if(inputAmt==0){
		$.toaster({ priority : 'danger', title : 'Token Alert !', message : "Please enter amount."});
		$('#amt').removeAttr("disabled","disabled");
	...
	if(inputAmt<0){
		$.toaster({ priority : 'danger', title : 'Token Alert !', message : "Please enter valid amount."});
		$('#amt').removeAttr("disabled","disabled");
		$(".depositRe").prop('disabled', true);
	...
	var formData  = "wallet_address="+userAddr+"&amount="+inputAmt+"&pId="+packageId;
	$.ajax({
		headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
		...
		success: function(result)
		{
			if(result.type !='error'){
				deposit_2( result.tokenAmt,result.amount,result.pId);
			}else{
...
const deposit_2  = async(tokenAmt,amount,pId) => {
	const { ethereum } = window;
	accounts = await ethereum.request({ method: 'eth_requestAccounts' });
	var userAddr = accounts[0];
	var chainId = await ethereum.request({ method: 'eth_chainId' });
	...
	Trade.methods.deposit(0,tokenAmt).send({from:userAddr},function(err, result){
		if(err!==null){
			...
			formData = "tokenAmt="+tokenAmt+"&trans_id=" + result + "&wallet_address="+userAddr+"&amount="+amount+"&pId="+pId;
			$.ajax({
				headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
...
const unstack = async(transId)=>{
	...
	if(chainId  != chainIdVal){
		$.toaster({ priority : 'danger', title : 'Chain Alert !', message : chainIdMsg });
		$(".unstack").removeClass("dis");
		return false;
	}
	var formData  = "wallet_address="+userAddr+"&transId="+transId;
    // alert(formData);
    // return false;
	// $.toaster({ priority : 'warning', title : 'Unstack Alert !', message :"Your request underprocess. Please wait.."});
	$.ajax({
		headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
		url: WEB_LINK+"unstack",
		data: formData,
		type: 'post',
		success: function(result)
		{
			if(result.type =='success'){
				$.toaster({ priority : 'success', title : 'Unstack Alert !', message :result.msg});
				setTimeout(function () {
					window.location.href = WEB_LINK+"users";
				}, 2000);
			}else{
				$.toaster({ priority : 'danger', title : 'Unstack Alert !', message : result.msg});
				$(".unstack").removeClass("dis");
			}
		}
	});
...
$(".withRe").on("click",function(){
	$(".withRe").addClass("dis");
	var inputAmt  = $("#withAmt").val();
	var grnadTotal  = $("#grnadTotal").val();
	var trans     = $(this).data("trans");
	var minAmt    = 2;

	if (inputAmt == 0) {
		$.toaster({ priority : 'danger',title : 'Required!',  message :"Please enter amount."});
		$(".withRe").removeClass("dis");
		return false;
	}

	if (inputAmt > grnadTotal) {
		$.toaster({ priority : 'danger',title : '!',  message :"Insufficient fund"});
		$(".withRe").removeClass("dis");
		return false;
	}
