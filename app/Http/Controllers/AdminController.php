...
class AdminController extends Controller
{
    ...
    public function roiIncome() {
        if(Auth::user() && Auth::user()->id==1){
            ...
            $transactions    = Transaction::where("trans",1)->orderBy("id","DESC")->get();
            return view('admin.roiIncome', compact('transactions') );
        }else{
            Session::flash('message', 'You are not a valid user.');
            Session::flash('alert-class', 'alert-danger');
            return redirect()->route('admin');
    ...
    public function levelIncome() {
        if(Auth::user() && Auth::user()->id==1){
            ...
            $roiList    = Transaction::where("trans",2)->orderBy("id","DESC")->get();
            // print_r($roiList);
            // die();
    ...
    public function leadershipIncome() {
        if(Auth::user() && Auth::user()->id==1){
            ...
            // $roiList      = Transaction::where("trans",2)->get();
            $leadershipList    = Transaction::where("trans",5)->get();
            return view('admin.leadershipIncome', compact('leadershipList') );
        }else{
            Session::flash('message', 'You are not a valid user.');
            Session::flash('alert-class', 'alert-danger');
            return redirect()->route('admin');
